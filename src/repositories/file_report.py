...
from src.db.models import FileReport
...
class FileReportRepository:
    ...
    @staticmethod
    async def read_files(db: AsyncSession) -> list[FileReport]:
        list_of_files = await db.execute(select(FileReport))
    ...
    async def read_file(
        ...
        file_to_read = await db.execute(
            select(FileReport).where(FileReport.id == file_id)
        )
    ...
    async def add_file_to_eco_problem(
            ...
            user:UUID,
            files: list[UUID],
            db: AsyncSession) -> list[FileReport]:
        res_list = []
        for f in files:
            res_list.append(await FileReportRepository.create_file(
                eco_problem_id,
                user,
                f,
                db
