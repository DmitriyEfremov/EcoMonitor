...
from src.repositories.file_report import FileReportRepository
from src.schemas.file_report import FileReportRead
...
router_files_report = APIRouter(prefix="/files_report", tags=["File report CRUD"])



@router_files_report.post(
    "/upload-files",
    response_model = list[FileReportRead]
)
async def add_file_to_report(
        report_id:int,
    ...
    return await FileReportRepository.add_file_to_eco_problem(
        report_id,
        user,
        storage_ids,
        db
...
async def read_files(
    ...
    return await FileReportRepository.read_files(db)


@router_files_report.get(
    "/{file_id}",
    response_model=FileReportRead
)
async def read_file(
        file_id: int,
        db: AsyncSession = Depends(get_db),
    ...
    return await FileReportRepository.read_file(file_id, db)


@router_files_report.delete(
    "/delete/{file_id}",
...
async def delete_file(
    ...
    await FileReportRepository.delete_file(file_id, db)
